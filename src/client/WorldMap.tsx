...
export function WorldMap({countries,cities,layer,showCities,onCountry,onCity}:Props){
  ...
  const fill=(country:Country|undefined)=>{if(!country?.visited)return country?.wishlisted?'var(--color-secondary-container)':'var(--color-surface-soft)';if(layer==='visited')return 'var(--color-primary-container)';const value=layer==='city_count'?Number(country.cityCount):Number(country.visitCount);return value>=6?'var(--color-primary)':value>=3?'color-mix(in srgb,var(--color-primary) 72%,var(--color-primary-container))':'var(--color-primary-container)';};
  ...
  return <div className="map-frame" aria-label="Interactive travel map">
    <svg viewBox="0 0 960 500" role="img" aria-labelledby="map-title map-desc" onWheel={e=>{e.preventDefault();changeZoom(zoom*(e.deltaY<0?1.18:.85));}}
      ...
      <defs><pattern id="wish" width="8" height="8" patternUnits="userSpaceOnUse"><path d="M0 8L8 0" stroke="var(--color-secondary)" strokeWidth="1"/></pattern></defs>
